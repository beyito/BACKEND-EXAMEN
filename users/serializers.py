...
class UserSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        user = User.objects.create_user(**validated_data, idRol=rol, password=password)
        return user
...
class MyTokenObtainPairSerializer(TokenObtainPairSerializer):
    def validate(self, attrs):
        ...
        data["rol"] = getattr(getattr(self.user, "idRol", None), "name", None)
        ...
        try:
            ...
            with transaction.atomic():
                ...
                cop = CopropietarioModel.objects.create(
                    idUsuario=user, unidad=validated_data["unidad"]
                )
                return cop
...
class ResidenteSerializer(serializers.ModelSerializer):
    ...
    copropietario_id = serializers.IntegerField(
        source="idCopropietario_id", read_only=True
    )
    ...
    def validate(self, attrs):
        ...
        if CopropietarioModel.objects.filter(idUsuario=user.id).exists():
            raise serializers.ValidationError(
                {"idUsuario": "El usuario ya es copropietario"}
            )
        # decide tu politica
        # a) estricto: exigir rol copropietario
        if not user.idRol or user.idRol.name != "Copropietario":
            raise serializers.ValidationError(
                {"idUsuario": "El usuario no tiene rol copropietario"}
            )
        attrs["user"] = user
        attrs["unidad"] = unidad
        return attrs
